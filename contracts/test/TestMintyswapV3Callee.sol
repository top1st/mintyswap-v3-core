...
contract TestMintyswapV3Callee is IMintyswapV3MintCallback, IMintyswapV3SwapCallback, IMintyswapV3FlashCallback {
    ...
    function swapToLowerSqrtPrice(
        ...
        IMintyswapV3Pool(pool).swap(recipient, true, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }
    ...
    function swapToHigherSqrtPrice(
        ...
        IMintyswapV3Pool(pool).swap(recipient, false, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }
    ...
    function mint(
        ...
        IMintyswapV3Pool(pool).mint(recipient, tickLower, tickUpper, amount, abi.encode(msg.sender));
    }
    ...
    function MintyswapV3MintCallback(
        ...
        if (amount0Owed > 0)
            IERC20Minimal(IMintyswapV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, amount0Owed);
        if (amount1Owed > 0)
            IERC20Minimal(IMintyswapV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
    }
    ...
    function flash(
        ...
        IMintyswapV3Pool(pool).flash(recipient, amount0, amount1, abi.encode(msg.sender, pay0, pay1));
    }
    ...
    function MintyswapV3FlashCallback(
        ...
        if (pay0 > 0) IERC20Minimal(IMintyswapV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
        if (pay1 > 0) IERC20Minimal(IMintyswapV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
    }
}
